    ...
    body('description', 'Description must be atleast 5 characters').isLength({ min: 5 })], fetchuser, async (req, res) => {
        ...
        try {
            ...
            if (note.user.toString() !== req.user.id) {
                return res.status(401).send("Not Allowed");   // login user only update his notes
            }

            note = await Note.findByIdAndUpdate(req.params.id, { $set: newNote }, { new: true })  // made changes
            res.json({ note });
        } catch (error) {
            console.error(error.message);
            res.status(500).send("Internal Server Error");
